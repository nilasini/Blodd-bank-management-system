...
public class AvailableNearbyDonors extends ListActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		Log.d("distance array length", String.valueOf(distanceArray.length));
		Log.d("RequestActivity.distanceList.size()", String.valueOf(RequestActivity.distanceList.size()));
	...
	public void addNearbyDonors(String[] distance) {
		...
		distanceInDouble = near.findNearbyDonors(distance);
		int x = 0;
		//check whether there are more then one available places to compare
		if (distanceInDouble.size() > 1) {
			
			if (distanceInDouble.get(0).equals(distanceInDouble.get(1))) {
				minDis = distanceInDouble.get(0).toString();
				HashMap<String, String> availableDonors = new HashMap<String, String>();
				Log.d("distance length", String.valueOf(distance.length));
				for (int y = 0; y < distance.length; y++) {
					Log.d("available donor list length",String.valueOf(sa.getAvailableDonorsList().size()));
			...
			minDis = distanceInDouble.get(x).toString();
			HashMap<String, String> availableDonors = new HashMap<String, String>();
			for (int y = 0; y < distance.length; y++) {
				availableDonors = sa.getAvailableDonorsList().get(y);
				if ((availableDonors.containsValue(minDis))) {
